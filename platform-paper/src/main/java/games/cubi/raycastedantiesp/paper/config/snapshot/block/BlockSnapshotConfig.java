package games.cubi.raycastedantiesp.paper.config.snapshot.block;
    ...
    public static class Factory implements ConfigFactory<BlockSnapshotConfig> {
        ...
        public @NotNull BlockSnapshotConfig getFromConfig(ConfigurationNode config) {
            ...
            return switch (mode) {
                ...
                default -> {
                    Logger.error(new RuntimeException("Unsupported block snapshot mode enum value: " + mode + ", falling back on sync-bukkit"), Frequency.CONFIG_LOAD.value);
