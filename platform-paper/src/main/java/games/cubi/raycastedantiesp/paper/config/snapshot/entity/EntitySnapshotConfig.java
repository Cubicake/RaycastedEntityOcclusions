...
public class EntitySnapshotConfig implements Config {
    ...
    public static class Factory implements ConfigFactory<EntitySnapshotConfig> {
        ...
        public @NotNull EntitySnapshotConfig getFromConfig(ConfigurationNode config) {
            ...
            if (mode == null) {
                Logger.warning("Invalid entity snapshot mode in config, defaulting to " + fallback.getMode().getName(), Frequency.CONFIG_LOAD.value);
            ...
            return switch (mode) {
                ...
                default -> {
                    Logger.error(new RuntimeException("Unsupported entity snapshot mode enum value: " + mode + ", falling back on bukkit"), Frequency.CONFIG_LOAD.value);
